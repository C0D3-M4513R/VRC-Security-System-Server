...
create or replace function change_logo(discord_id bigint, "club-path-name" text, image bytea) returns void
    language sql
as
$$
...
CREATE or replace function change_poster1(
    discord_id bigint,
    "club-path-name" text,
    image bytea
) returns
...
CREATE or replace function change_poster2(
    discord_id bigint,
    "club-path-name" text,
    image bytea
) returns
...
CREATE or replace function change_poster3(
    discord_id bigint,
    "club-path-name" text,
    image bytea
) returns
...
create or replace function change_club_name(
    discord_id bigint,
    "club-path-name" text,
    name text
) returns
...
WITH valid as (
    ...
    FROM club, vrc_name
       INNER JOIN discord_permissions ON
        (id = discord_permissions.club_id OR discord_permissions.club_id = 0) AND
